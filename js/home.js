...
/*
function insert_add_form_admin() {
    
    $form = $("<form>", {class : "border border-primary align-self-start"}); 
    $img = $("<img>", {alt : "[New Item Image]", class : "form-group"}); 
    add_upload_func($img); 
    $form.append($("<div>", {class : "form-group"})); 
    $form.children("div:last-of-type").append($img); 
    var $name = $("<input>", {class : "form-control", type : "text", placeholder : "Item Name" }); 
    var $desc = $("<textarea>", {class : "form-control", rows : "3", placeholder : "Description"}); 
    $form.append($("<div>", {class : "form-group"})); 
    $form.children("div:last-of-type").append($name);
    $form.append($("<div>", {class : "form-group"}));
    $form.children("div:last-of-type").append($desc);

    var $price_row = $("<div>", {class : "input-group mb-3"}); 
    $price_row.append($("<div>", {class : "input-group-prepend"}));
    $price_row.find("div").append($("<span>", {class : "input-group-text", text : "$"})); 
    var $price = $("<input>", {class : "form-control", type : "number", step : "0.01", min : "0.00", val : "0.00"}); 
    $price_row.find("div:last-of-type").append($price); 
    $form.append($("<div>", {class : "form-group"})); 
    $form.children("div:last-of-type").append($price_row);

    $form.append($("<div>", {class : "form-group"})); 
    $form.children("div:last-of-type").append($("<input>", {type : "submit", class : "btn btn-primary", val : "Add New Item!"}));
    
    $error_div = $("<div>", {class : "form-group"}); 
    $error_div.append($("<p>", {class : "text-danger"})); 
    $form.append($("<div>", {class : "form-group"})); 
    $form.children("div:last-of-type").append($("<p>", {class : "text-danger"}));
    $error = $("<strong>", {class : "text-danger text-center"}); 
    $form.find("div:last-of-type p").append($error); 
